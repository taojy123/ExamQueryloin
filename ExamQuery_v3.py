...
for i in range(1, table.nrows):
    try:
        ...
        if "合格人员登记" in p:
            ...
            infoid = re.findall(r'id="hidInfoID" value="(.*)" />', p)[0]
            print infoid

            p = get_page("http://www.ynrsksw.cn/Ynrsks/ExamineeRecord/ExamineeInfo2.aspx?TableNO=62&RegType=0&ID=%s"%infoid)
            
            phone = re.findall(r'name="txtTeleNo" type="text" value="(.*)" id="txtTeleNo"', p)[0]
            
            print phone
            
            result = name + ",'" + idnum + "," + zy + ",'" + phone

    except:
        print "err", name
        traceback.print_exc()
        result = name + ",'" + idnum + "," + zy + ",Null"
